...
const DEFAULT_URL = 'http://daringfireball.net/feeds/articles'
...
export default class Explore extends React.Component {
  ...
  renderFeed(){
    debugger;
    if(this.props.atom.result && this.props.atom.result.result && this.props.atom.result.result.feed && this.props.atom.result.result.feed.entries)
    ...
    return (
      <div className="content">
        <form className="explore pure-form" onSubmit={e => e.preventDefault()}>
          <fieldset>
            <legend> Atom Feeds Browser </legend>
            <input
              size="45"
              ref="url"
  ...
  handleGoClick () {
    ...
    this.context.history.pushState({}, `/feed/${encodeURIComponent(this.getInputValue())}`)
  }
